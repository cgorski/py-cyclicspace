...
def split_chunks(num: int, part_size: int):
    uniform_parts = (num+1)//part_size
    remainder = (num+1)%part_size
    parts = []
    placeholder = 0

    for _ in range(uniform_parts):
        part = [placeholder, placeholder + (part_size-1)]
        placeholder += part_size
        parts.append(part)

    if remainder != 0:
        parts.append([placeholder, placeholder + (remainder-1)])
...
@jit(nopython=True, nogil=True)
def calc_cycle(o_uni, n_uni, nv, w, h, chunk1, chunk2):
    for y in range(chunk1, chunk2):
        for x in range(0,w):
...
class Universe:
    ...
    def next_cycle(self) -> "Universe":
        ...
        chunks = split_chunks(w, 200)
        for chunk in chunks:
            print("({},{})".format(chunk[0],chunk[1]))
        threads = [threading.Thread(target=calc_cycle, kwargs={'o_uni':o_uni, 'n_uni':n_uni,'nv': nv, 'w': w, 'h':h,'chunk1':chunk[0], 'chunk2':chunk[1]}) for chunk in chunks]
#            calc_cycle(o_uni = o_uni, n_uni = n_uni, nv = nv, w = w, h = h, chunks=c)
...
def main():
    ...
    uni = Universe(num_values=16, width=1024, height=1024, random=True)
    b = 0
    for i in uni.iter():
        b = b+1
